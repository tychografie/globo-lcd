...
.wordmark{display:flex;align-items:center;gap:10px;font-size:12px;font-weight:900;letter-spacing:.42em;color:var(--ink);margin-bottom:22px}
...
/* ---------- offline state ---------- */
/* Losing the device is a state, not a footnote: the whole remote greys out
   and a full-ink banner says so. */
.rec{position:fixed;top:calc(env(safe-area-inset-top) + 12px);left:50%;transform:translate(-50%,-64px);z-index:60;
  padding:12px 22px;border-radius:99px;background:var(--ink);
  font-size:13px;font-weight:800;letter-spacing:.08em;text-transform:uppercase;color:var(--bgc);
  box-shadow:0 8px 26px rgba(0,0,0,.35);white-space:nowrap;
  transition:transform .35s cubic-bezier(.3,1.2,.4,1)}
.rec.show{transform:translate(-50%,0)}
.offline main{opacity:.4;filter:saturate(.25);pointer-events:none;transition:opacity .4s ease,filter .4s ease}
...
<div class="grain"></div>
<div class="rec" id="rec">not connected</div>
...
function poll(){
  api("/api/status").then(function(s){
    fails=0;$("#rec").classList.remove("show");
